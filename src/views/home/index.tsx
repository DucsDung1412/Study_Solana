              ...
              <div className="relative grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="text-default-300 md:text-lg">
                    Lauch your solana toke, All in one solana token development and deployment
                  </p>

                  <div className="new_add_css">
                    <a onClick={() => setOpenCreateModal(true)} className='hover:bg-primary-hover pe-4 group mt-10 inline-flex items-center justify-center gap-2 rounded-full border border-white/10 px-1 text-white transition-all duration-500'>
                ...
                <div className="mx-auto h-[595px] overflow-hidden">
                  <div className="marquee grid grid-cols-2 gap-6">
                    <div className="relative m-auto flex flex-col gap-6 oveflow-hidden">
                      <div className="marquee-hero flex min-h-full flex-shrink-0 flex-col items-center justify-around gap-6">
                        {
                          ["img-9", "img-14", "img-21", "img-22", "img-10"].map((item, index) => (
                            <img key={index} src={`assets/images/ai/${item}.jpg`} alt="" className='aspect-1 h-full w-60 rounded-xl object-cover' />
                          ))
                        }
                      </div>

                      <div aria-hideen="true" className="marquee-hero flex min-h-full flex-shrink-0 flex-col items-center justify-around gap-6">
                        {
                          ["img-9", "img-14", "img-21", "img-22", "img-10"].map((item, index) => (
                            <img key={index} src={`assets/images/ai/${item}.jpg`} alt="" className='aspect-1 h-full w-60 rounded-xl object-cover' />
                          ))
                    ...
                    <div className="marquee-reverse m-auto flex flex-col gap-6 overflow-hidden">
                      <div className="marquee-hero flex min-h-full flex-shrink-0 flex-col items-center justify-around gap-6">
                        {
                          ["img-6", "img-10", "img-11", "img-12", "img-13"].map((item, index) => (
                            <img key={index} src={`assets/images/ai/${item}.jpg`} alt="" className='aspect-1 h-full w-60 rounded-xl object-cover' />
                          ))
                        }
                      </div>
